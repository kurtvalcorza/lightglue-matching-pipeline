...
WORKSHOP_NOTEBOOK_NAME = "DIMER_MultiModel_Image_Matching_Workshop.ipynb"
EXPECTED_PROFILE = "E2E"
EXPECTED_MODEL_ID = "cvg/LightGlue"
...
EXPECTED_OUTPUTS = (
    ...
    "outputs/lightglue_matching_input_manifest.json",
    "outputs/lightglue_matching_evaluation_report.json",
    "outputs/lightglue_matching_shapes.json",
    "outputs/lightglue_matching_adapter",
    "outputs/lightglue_matching_result.json",
...
MARKDOWN_MARKERS = (
    "**Capability:** sparse local-feature matching (ALIKED keypoints and descriptors matched by the LightGlue transformer, one confidence per correspondence), homography-supervised evaluation and bounded supervised fine-tuning of the matcher's last layers and assignment head",
    "**Apache-2.0**; ",
...
FORBIDDEN_PATTERNS = (
    ...
    (
        ...
        # A `torch.load(` without `weights_only=True` is the unsafe form; the carried `model.py` unpickles
        # the audited sources exactly once with `weights_only=True` (the PubMedCLIP / CFNet precedent).
        re.compile(r"\bpickle\.load|\btorch\.load\s*\((?![^)]*weights_only=True)|getattr\(\s*torch\s*,\s*['\"]load['\"]"),
    ),
...
def _package_identity() -> tuple[str, str]:
    ...
    # The revision must be immutable. LightGlue publishes no Hub repository: the matcher is an asset of a
    # GitHub release tag, immutable in the same sense (and pinned by SHA-256 in the manifest); the YOLOX
    # row set the precedent. `main` / `latest` stay refused and the notebook pattern still catches them.
    RELEASE_TAG = re.compile(r"^v?\d+\.\d+(?:\.\d+)?(?:rc\d+|_[a-z0-9]+)?$")
    _check(
...
def _validate_notebook_structure(path: Path, notebook: dict) -> tuple[list[tuple[int, str, ast.Module]], str]:
    ...
    markdown = "\n".join(markdown_parts)
    # The vendored modeling.py is excluded from the placeholder scan only (the YOLOX precedent): the
    # G9/PLACEHOLDER rule catches unfinished *authored* content, and upstream LightGlue's own
    # `# TODO: Remove when hloc switches to the compact format.` is neither unfinished nor ours to edit.
    vendored_cells = {
        index
        for index, cell in enumerate(notebook.get("cells", []))
        if Path(cell.get("metadata", {}).get("dimer", {}).get("embedded_module", "")).name == "modeling.py"
    }
...
def _validate_notebook_content(
    ...
    _check(
        ...
        f"{path.name}: references must link {model_id}",
    )
