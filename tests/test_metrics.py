...
from lightglue_pipeline.samples import warp_image
...
def test_mutual_nn_matches_pairs_only_mutual_neighbours():
    d0 = np.array([[1.0, 0.0], [0.0, 1.0], [0.7, 0.7]])
    d1 = np.array([[0.0, 1.0], [1.0, 0.0]])
    pairs, sims = mutual_nn_matches(d0, d1)
    assert pairs.tolist() == [[0, 1], [1, 0]] and np.allclose(sims, 1.0)
    pairs2, _ = mutual_nn_matches(d0, d1, min_similarity=1.5)
    assert len(pairs2) == 0
    empty, _ = mutual_nn_matches(np.zeros((0, 2)), d1)
    assert empty.shape == (0, 2)
    with pytest.raises(ValueError, match="common D"):
        mutual_nn_matches(d0, np.zeros((2, 3)))
