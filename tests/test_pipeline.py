...
from lightglue_pipeline import pipeline as pl
...
def test_validate_inputs_reports_and_refuses_before_the_model(tmp_path, forbid_model_imports):
    ...
    assert manifest["verdict"] == "accepted" and manifest["images"][0]["size"] == [200, 150]
    assert manifest["schema"]["thresholds"] == {"match": 0.1} and manifest["schema"]["keypoints"]["max_per_image"] == 2048
...
def test_extract_and_match_return_the_contract_structure():
    pipe = _pipe()
    image = textured_image(1, (160, 120))
    feats = pipe.extract(image)
    assert feats["keypoints"].shape[1] == 2 and feats["descriptors"].shape == (len(feats["keypoints"]), 128)
    assert feats["scores"].shape == (len(feats["keypoints"]),) and feats["size"] == [160, 120]
    assert np.allclose(np.linalg.norm(feats["descriptors"], axis=1), 1.0, atol=1e-4)
    ...
    assert result["kpts0"].dtype == np.float32 and result["n_keypoints0"] == len(feats["keypoints"]) and result["layers"] == 9
    nn = pipe.descriptor_nn_match(image, image)
    assert nn["kpts0"].shape == nn["kpts1"].shape and "mutual nearest neighbour" in nn["baseline"]
    with pytest.raises(ValueError, match="sides must lie"):
...
def test_evaluate_and_baselines_share_the_scoring_code():
    ...
    assert set(baselines) == {"identity", "patch_neighbour", "descriptor_nn"} and baselines["identity"]["matcher"] == "baseline"
...
def test_adapt_validates_arguments_before_touching_the_model():
    ...
    with pytest.raises(ValueError, match="1..9"):
        pipe.adapt(records, trainable_layers=0)
    ...
    assert pipe.adapter is None and not any(p.requires_grad for p in pipe.matcher.parameters())
...
def test_one_epoch_adaptation_round_trip_on_the_random_network(tmp_path):
    ...
    extractor_before = {k: v.clone() for k, v in pipe.extractor.state_dict().items()}
    # without a validation split the final epoch is kept, so the trained tensors must differ from the base
    result = pipe.adapt(records[:4], None, epochs=1, lr=1e-4, batch_size=2, trainable_layers=1)
    assert result["n_trainable"] == 1_250_560 + 66_049 and result["history"][0]["note"] == "frozen model"
    assert result["history"][1]["train_loss"] > 0 and result["best_epoch"] == 1
    assert result["selection"].startswith("final epoch") and result["ground_truth"]["positive_px"] == 3.0
    changed = [k for k, v in pipe.matcher.state_dict().items() if not torch.equal(v, before[k])]
    assert changed and all(k.startswith(("transformers.8.", "log_assignment.8.")) for k in changed)
    assert all(torch.equal(v, extractor_before[k]) for k, v in pipe.extractor.state_dict().items())
    ...
    assert manifest["format"] == ARTIFACT_FORMAT and manifest["base_model"]["weight_sha256"] == MATCHER_SHA256
    assert manifest["base_model"]["extractor_sha256"] == EXTRACTOR_SHA256
    assert len(manifest["tensors"]) == len(result["trainable_names"]) and manifest["metadata"] == {"note": "test"}
    fresh = _pipe()
    fresh.load_artifact(artifact)
    for k in changed:
        assert torch.equal(fresh.matcher.state_dict()[k], pipe.matcher.state_dict()[k])
    assert fresh.adapter["best_epoch"] == result["best_epoch"]
    # with a validation split the epoch is selected on precision at 3 px and the selection rule is recorded
    again = _pipe()
    selected = again.adapt(records[:4], records[4:], epochs=1, lr=1e-4, batch_size=2, trainable_layers=1)
...
def test_load_artifact_rejects_bad_manifests_before_touching_weights(tmp_path):
    ...
    manifest = {
        ...
        "base_model": {"id": MODEL_ID, "revision": MODEL_REVISION, "weight_sha256": MATCHER_SHA256},
    ...
    with pytest.raises(ValueError, match="different base model"):
        pipe.load_artifact(tmp_path)
    write({**manifest, "base_model": {**manifest["base_model"], "extractor_sha256": "0" * 64}})
    with pytest.raises(ValueError, match="different base weight file or extractor"):
        pipe.load_artifact(tmp_path)
    write({**manifest, "tensors": ["input_proj.weight"]})
